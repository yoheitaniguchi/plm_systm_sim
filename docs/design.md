...
| PLM-EXT-12 | 影響分析タブでの原価影響（7.7）の見せ方 | `computeCostImpact()`はECOのbefore/after BOMを引数に取るが、UI側で「ECOごとのBOMスナップショットを保持する」具体的な仕組みは要件書に無い | 簡易版として、**代替部品グループのオーバーライド適用前後**（デフォルト優先順位のM-BOM vs 現在のオーバーライド適用後のM-BOM）を比較する形でUIに表示する。この機能は引き続き維持する（`ImpactTab.tsx`「原価影響の簡易算出」）。ECOごとのbefore/afterスナップショット管理はPLM-EXT-14として実装した（Issue #4） |
...
| PLM-EXT-14 | ECOのbefore/after BOM比較（Issue #4、PLM-EXT-12の先送り事項） | Issue #4は「ECRごとに起票時点のBOMスナップショットを保持し、クローズ前後の原価影響を確認できるようにする」ことを求めるが、スナップショットを取得するタイミング・対象BOM種別・保存方法は要件書に規定が無い | `EngineeringChange.beforeBomLines`（`PlmBomLine[]`）を追加し、**`change/create`ディスパッチ時点の`reducer.effectiveMbomLines()`（代替部品グループ解決済みの実効M-BOM）を`structuredClone`して記録する**。E-BOM側は既存方針（PLM-EXT-12・7.7）を踏襲し対象外。afterBom側は専用スナップショットを持たず、参照時点の`effectiveMbomLines(state)`（現在のM-BOM）をそのまま使う——本シミュレーターは永続化が無い単一セッションであり、ECOクローズ後もM-BOM側の実変更（代替部品切替・E-BOM改訂→再変換等）はユーザーが別操作で行うため、「クローズ時点で自動的に確定するafter」という概念自体が存在しないため。UIは`ImpactTab.tsx`に既存の代替部品前後比較セクションとは別枠で「ECRごとの原価影響（起票時点比較）」を追加し、`beforeBomLines`を持つ変更を選択して`computeCostImpact()`の結果を表示する形にした（既存機能は置き換えず維持）。**既知の境界事例**：対象品目がECR起票時点でまだM-BOMへ変換されていない場合、`beforeBomLines`は空配列になり「起票時点の原価＝0円」として表示される。これは`assertMbomConverted()`のような未変換エラーではなく、「その時点でM-BOM原価がまだ存在しなかった」という事実をそのまま表す仕様とする（発注BOM・計画BOM＝PLM-EXT-13が未変換を拒否するのとは目的が異なり、こちらは時系列の変化を見せる機能のため、変換前後をまたぐ場合でも黙って0円からの差分として表示してよい） |
